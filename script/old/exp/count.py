...
def count_all(p, db, start, end):
  ...
  atwi_path = result_dir + '/count/' + p + '_all.txt'
...
def main():
  for p in ['is', 'tk']:
    ...
    count_all(p, db, start, end)

    rates = range(10, 101, 10)
    # rates = [0]
    for ext in ['pmi', 'soa']:
      print('### ' + ext)
      filename = result_dir + 'relation_word/' + p + '_' + ext + '.txt'
      for rate in rates:
        print('###\t' + str(rate))
        ...
        for date in daterange(start, end):  
          ...
          if len(words) > 0:
            ...
            stwi_count = len(stwis)
          stwi.append('\t'.join([m, d, str(stwi_count)]))

        stwi_path = result_dir + '/count/' + p + '_s_' + ext + '_' + str(rate) + '.txt'
